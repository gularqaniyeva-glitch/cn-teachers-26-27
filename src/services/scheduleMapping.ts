...
const SCHEDULE_FIELD_CANDIDATES = {
  ...
  // "Açılması" — точное название реальной колонки даты открытия (проверено
  // напрямую через /api/sheets). "Açılmasını yoxla" (буквально "проверь его
  // открытие") — ОТДЕЛЬНАЯ, самостоятельная колонка-заметка, а не синоним:
  // на реальных данных её значение может расходиться с "Açılması" и с явным
  // флагом "Открыть курс" (напр. M4 5-9: "Открыть курс"=TRUE и
  // "Açılması"=сегодня, но "Açılmasını yoxla"=будущая дата) — раньше она
  // была первым кандидатом и подставляла неверную дату. "Açılma tarixi"
  // на практике всегда дублирует "Açılması" (то же значение в каждой
  // строке) и оставлена запасным вариантом.
  openDate: [
    'Açılması',
    'Açılma tarixi',
    'Açılış tarixi',
