...
function tierFor(passRate: number, started: number): keyof typeof TIER_STYLES {
  if (started === 0) return 'low';
...
export function ModuleHeatmapGrid({ modules, passedLabel, ofLabel, emptyLabel }: ModuleHeatmapGridProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
      {modules.map((m) => {
        const tier = tierFor(m.passRate, m.started);
        return (
          <div key={m.moduleId} className={`rounded-xl border p-3 ${TIER_STYLES[tier]}`}>
            ...
            <p className="mt-1 text-2xl font-bold">{m.started > 0 ? `${m.passRate}%` : '—'}</p>
            <p className="mt-0.5 text-xs opacity-70">
              {passedLabel} {m.passed} {ofLabel} {m.started}
            </p>
          </div>
        );
      })}
